...
function createMesh() {
    return new Promise(function (resolve, reject) {
        ...
        // legend.innerHTML = 'Высота поверхности = модуль <br>цвет поверхности - аргумент выдачи (R→G→B)';
        let graph_type = 4;
        if (plot == 'Реал-Вым') {
            if (isBW) graph_type = 1;
            else graph_type = 0;
        } else if (plot == 'Вым-Реал') {
...
function createText() {
    ...
    loader.load('fonts/CMU.json', function (font) {
        ...
        let Imlabel = new THREE.TextGeometry('Вым', options);
        materials = [
        ...
        let Relabel = new THREE.TextGeometry('Реал', options);
        materials = [
            new THREE.MeshPhongMaterial({ color: null, flatShading: true }), // front
            new THREE.MeshPhongMaterial({ color: null }) // side
        ];
